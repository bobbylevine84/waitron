<div class="container">
    ...
    <div class="main-map">
        ...
        <script type="text/javascript">
              ...
              var isDraggable,isScrollable = width > 480 ? true : false;
              var mapOptions = {
                zoom: 4,
                scrollwheel: false,
                ...
                <table width="100%" border="0">
                    ...
                    <?php foreach ($ujobs as $ujob) { ?>
                      <tr>
                        ...
                        <td> <?php if($ujob->jobtype=='Event') { ?> <a data-toggle="ajaxModal" href="<?=base_url()?>jobs/status/<?=$ujob->jobid?>">status</a> <?php } else { ?> <img src="<?=base_url()?>resource/images/<?php if($staff->job_accept_status=='booked' && $staff->confirm_status=="") { echo 'status-blue.png'; } elseif($staff->job_accept_status=='booked' && $staff->confirm_status=="yes") { echo 'status-green.png'; } else { echo 'status-red.png'; } ?>" alt="" title="<?=$staff->job_accept_status?>"> <?php } ?> </td>
                        <td><a href="<?=base_url()?>jobs/update<?=$ujob->jobtype=='Job' ? 'job' : 'event'?>/<?=$ujob->jobid?>">edit</a></td>
                        <td><?php if($ujob->cancel_status=='no') { ?><a href="<?=base_url()?>jobs/cancel/<?=$ujob->jobid?>/no" data-toggle="ajaxModal"> cancel</a> <?php } else { ?> <span style="color: red;">canceled</span> <?php } ?>  </td>
                      </tr>
                    <?php } ?>
                    </tbody> 
                </table>
