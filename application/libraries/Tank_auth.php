...
class Tank_auth
{
			...
			if (!is_null($user = $this->ci->users->$get_user_func($login))) {	// login ok
				...
				if ($hasher->CheckPassword($password, $user->password)) {		// password ok
					...
					if ($user->banned == 1) {									// fail - banned
						$user->ban_reason==''? $reason='Your Account Deleted By Admin.' : $reason='Your Account Deleted By Admin. Reason:- "'.$user->ban_reason.'"';
						$this->error = array('banned' => $reason);
	...
	function hashed_pass($pass)
	{
		...
		$hasher = new PasswordHash(
				$this->ci->config->item('phpass_hash_strength', 'tank_auth'),
				$this->ci->config->item('phpass_hash_portable', 'tank_auth'));
		return $hashed_password = $hasher->HashPassword($pass);
		//return $this->ci->session->userdata('user_type');
	}
	 ...
	 * @return	array
	 */
	function create_user($username, $email, $password, $fullname, $role, $parent, $email_activation)
	{
	...
	function change_password($old_pass, $new_pass)
	{
		...
		if (!is_null($user = $this->ci->users->get_user_by_id($user_id, TRUE))) {
			...
			if ($hasher->CheckPassword($old_pass, $user->password)) {			// success
				...
				$this->error = array('old_password' => 'Enter a Valid Current Password');
			}
		}
		return FALSE;
	}
	...
	/**
	 * Get permission id by current page slug 
	 */
	
	function get_permission_by_slug($slug){
		$this->ci->db->where('slug', $slug);
		$data = $this->ci->db->get('permissions')->row_object();
		if($data){
	...
	/**
	 * set page access permission according to current user role
	 */
	
	function get_role_permission($permission_id,$roleid ){
		$count=$this->ci->db->where(array('role_id'=>$roleid,'permission_id'=>$permission_id))->get('role_permissions')->num_rows();
		if ($count==1){
			return TRUE;
		}
		else{
	...
	function getRolepermission()
	{
		if ($this->get_role_id() !=1) {
			$userRole=$this->get_role_id();
			$this->ci->db->where('role_id',$userRole);
			return $this->ci->db->get('role_permissions')->result();
		}
	}
	...
	function checkPermission($slug)
	{
		...
		if($userRole!=1)
		{
		$permissionID=$this->get_permission_by_slug($slug);
		return $this->get_role_permission($permissionID, $userRole);
		die('dsasd');
		}
		else
		{
	...
	function get_role_name($id){
		$data = $this->ci->db->where('r_id', $id)->get('roles')->row_object();;
		if($data){
			return $data->role;
		}
	}
